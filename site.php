...
$app->post('/register', function () {
    ...
    if (!isset($_POST['email']) || $_POST['email'] == '') {
        User::setErrorRegister('Informe seu e-mail');
        header('Location: /login');
        exit;
    }

    if (!isset($_POST['password']) || $_POST['password'] == '') {
        User::setErrorRegister('Informe a senha');
        header('Location: /login');
        exit;
    }

    if (User::checkLoginExist($_POST['email']) === true) {
        User::setErrorRegister('Já existe um usuário com esse e-mail');
        header('Location: /login');
        exit;
    }
...
$app->get('/boleto/:idorder', function ($idorder) {
    ...
    $totalCarrinho = formatPrice($order->getvltotal());
    $totalCarrinho = str_replace('.', '', $totalCarrinho);
    $valor_cobrado = $totalCarrinho; // Valor - REGRA: Sem pontos na milhar e tanto faz com "." ou "," ou com 1 ou 2 ou sem casa decimal
...
$app->post('/profile/change-password', function () {
    ...
    if ($_POST['current_pass'] === $_POST['new_pass']) {
        User::setError('Nova senha deve ser diferente da senha atual');
        header('Location: /profile/change-password ');
        exit;
    }
